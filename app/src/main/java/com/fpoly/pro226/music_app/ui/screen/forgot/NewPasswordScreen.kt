...
@Composable
fun NewPasswordScreen(onBack: () -> Unit, appContainer: AppContainer) {
    val extras = MutableCreationExtras().apply {
        set(ForgotViewModel.MY_REPOSITORY_KEY, appContainer.fMusicRepository)
    }
    ...
    LaunchedEffect(vm.forgotUiState) {
        vm.toastEvent.collect { message ->
            Toast.makeText(context, message, Toast.LENGTH_SHORT).show()
        }
    }
    LaunchedEffect(vm.forgotUiState.isNewPasswordSuccessfully) {
        if (vm.forgotUiState.isNewPasswordSuccessfully == true) {
            onBack()
        }
    }
